    ...
    <div class="d-flex flex-column  justify-content-between h-100">
        ...
        <div class="content bg-blur h-100 overflow-auto bg-white">
            <div v-for="(user, index) in filteredusers" :key="index">
                ...
                >
                    <div
                        class="d-flex justify-content-between align-items-center pt-2 mx-2"
                    >
                        <div class="circle-sm">
                            <img
                                v-bind:src="getUserAvatar(user)"
                                class="pok-trade"
                            />
                        </div>
                        <h2 class="d-flex">{{ user.username }}</h2>
                        <div class="pok-id align-items-end">
                            ID {{ getUserId(user) }}
                        </div>
                    </div>
                </router-link>
                <hr />
                <div
                    v-for="(pok, index) in getPoksTeam(user.team)"
                    :key="index"
                >
                    <router-link
                        class=""
                    ...
                    >
                        <div
                            class="d-flex flex-row flex-nowrap justify-content-between my-2 mx-4"
                        >
                            <img
                                v-bind:src="pokImage(pok)"
                                :alt="pok.name"
                                class="pok-sm"
                            />
                            ...
                            <div>
                                <img
                                    v-bind:src="typeImage(pok.types.type1)"
                                    :alt="pok.types.type1"
                                    class="pok-type"
                                />
                                <img
                                    v-if="pok.types.type2"
                                    v-bind:src="typeImage(pok.types.type2)"
                                    :alt="pok.types.type2"
                                    class="pok-type"
                                />
                            </div>
    ...
    methods: {
        getPoksTeam(pokTeam) {
            ...
            pokTeam.forEach(element => {
                let pok = this.$store.getters.getPoks.find(
                    item => item.id == element.pokemon_id
                );
                teamPoks.push(pok);
            });
            return teamPoks;
